...
export function AppShell({
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-40 flex w-64 -translate-x-full flex-col border-r border-[var(--line)] bg-white transition-transform lg:sticky lg:top-0 lg:h-screen lg:translate-x-0",
          open && "translate-x-0",
        )}
      >
        ...
        {/* Account block sits directly beneath the logo so the name and the
            sign-out control stay reachable without a long run of dead space
            between them and the navigation. */}
        <div className="shrink-0 border-b border-[var(--line)] p-3">
          <div className="flex items-center gap-3 rounded-lg bg-slate-50 px-3 py-2.5">
            ...
            <button
              ...
              title="Sign out"
              aria-label="Sign out"
              className="shrink-0 rounded-md p-1.5 text-slate-400 transition hover:bg-white hover:text-red-600 disabled:opacity-50"
            >
        ...
        {/* Only the navigation scrolls, so a long menu never pushes the
            account block off-screen. */}
